...
public class Main extends MovieClip {
  ...
  private function onDownloadSymbolUp(e:MouseEvent) {
    ...
    if (e.type == "mouseUp") {
      ...
      if (os.indexOf("iPhone") < 0) {
        // android
        source = File.applicationDirectory.resolvePath("./media/mp3_64/" + titleHeader + playList[playIndex] + ".m4r");
        target = File.documentsDirectory.resolvePath("./media/audio/ringtones/" + titleHeader + playList[playIndex] + ".m4r");
  ...
  private function onEnterFrame(e:Event) {
    if (currentFrame == 20 || currentFrame == 40 || currentFrame == 50 || currentFrame == 60) {
      prevPlayIndex = playIndex;
      stop();
      if (currentFrame == 50 || currentFrame == 60) {
        stage.addEventListener(MouseEvent.CLICK, onBackgroundClick);
      }
    }
  }

  private function onBackgroundClick(e:MouseEvent) {
    gotoAndStop(20);
